...
class Rotation:
    ...
    def __init__(self, master, idet='D1'):
        ...
        self.detNuPos.set('%.3f' % mDetnu.RBV)
        if idet == 'D1':
            self.collect.set(1)
            self.wide.set(1)
        ...
        if idet == 'D1':
            self.det_head = Label(self.frame, text='Detector Del')
            self.det_head.grid(row=0, column=1)
            self.detNu_head = Label(self.frame, text='Detector Nu')
            self.detNu_head.grid(row=0, column=2)
            self.start_head = Label(self.frame, text='Start')
            self.start_head.grid(row=0, column=4)
            self.range_head = Label(self.frame, text='Range')
            self.range_head.grid(row=0, column=5)
            self.end_head = Label(self.frame, text='End')
            self.end_head.grid(row=0, column=6)
            self.steps_head = Label(self.frame, text='    Steps')
            self.steps_head.grid(row=0, column=7)
            self.size_head = Label(self.frame, text='Step size')
            self.size_head.grid(row=0, column=8)
            self.tPerDeg_head = Label(self.frame, text='time/degree')
            self.tPerDeg_head.grid(row=0, column=9)
            self.num_wide_head = Label(self.frame, text='Wides')
            self.num_wide_head.grid(row=0, column=12)
        ...
        self.entry_detNuPos = Entry(self.frame, textvariable=self.detNuPos,
                                    width=10)
        self.entry_detNuPos.grid(row=1, column=2, padx=5)
        self.entry_detNuPos.bind('<FocusOut>', self.detnu_pos_validation)
        self.entry_detNuPos.bind('<Return>', self.detnu_pos_validation)
        self.check_detCol = Checkbutton(self.frame, text='collect',
                                        variable=self.collect, width=10)
        self.check_detCol.grid(row=1, column=3, padx=5)
        self.entry_wStart = Entry(self.frame, textvariable=self.wStart,
                                  width=10)
        self.entry_wStart.grid(row=1, column=4, padx=5)
        ...
        self.entry_wRange = Entry(self.frame, textvariable=self.wRange,
                                  width=10)
        self.entry_wRange.grid(row=1, column=5, padx=5)
        ...
        self.label_wEnd.grid(row=1, column=6, padx=5)
        self.entry_nPTS = Entry(self.frame, textvariable=self.nPTS,
                                width=10)
        self.entry_nPTS.grid(row=1, column=7, padx=(20, 5))
        ...
        self.label_stepSize = Label(self.frame, textvariable=self.stepSize,
                                    width=8, relief=SUNKEN, anchor='w')
        self.label_stepSize.grid(row=1, column=8, padx=5)
        self.entry_tPerDeg = Entry(self.frame, textvariable=self.tPerDeg,
                                   width=10)
        self.entry_tPerDeg.grid(row=1, column=9, padx=20)
        ...
        self.check_wide = Checkbutton(self.frame, text='Wide',
                                      variable=self.wide)
        self.check_wide.grid(row=1, column=10, padx=5)
        self.check_steps = Checkbutton(self.frame, text='Steps',
                                       variable=self.steps)
        self.check_steps.grid(row=1, column=11, padx=5)
        self.entry_num_wide = Entry(self.frame, textvariable=self.num_wide, width=10)
        self.entry_num_wide.grid(row=1, column=12, padx=5)
    ...
    def detnu_pos_validation(self, event):
        # value must be float
        # in this case, also must lie within backlash-affected limits
        try:
            val = self.detNuPos.get()
            isinstance(val, float)
            back_min = val - mDetnu.BDST
            back_max = val + mDetnu.BDST
            if mDetnu.within_limits(back_min) and mDetnu.within_limits(back_max):
                pass
            else:
                self.detNuPos.set(mDetnu.RBV)
                limits_warn()
        except ValueError:
            self.detNuPos.set(mDet.RBV)
            invalid_entry()
...
# hard-encoded configuration options
    ...
    # bnc = PV('16TEST1:cmdReply1_do_IO.AOUT')
    # bnc_channel = 's02'
elif config.stack_choice.get() == '13BMC':
    mX = Motor('13BMC:m44')
    mY = Motor('13BMC:m45')
    mZ = Motor('13BMC:m46')
    mW = Motor('13BMC:m33')
    mDet = Motor('13BMC:m37')
